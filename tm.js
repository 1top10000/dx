...
for (let i = 0; i < gx; i++) {
    ...
    for (let j = 0; j < gy; j++) {
        ...
        for (let f_ of oj) {
            let ds; const pnt = bc(f_.pos.x, f_.pos.y, f_.pos.z, f_.size.x, c.x, c.y, c.z, d.i, d.j, d.k); if (pnt[6] !== 'x') { const ds0 = dst(pnt[0], pnt[1], pnt[2], c.x, c.y, c.z); const ds1 = dst(pnt[3], pnt[4], pnt[5], c.x, c.y, c.z); ds = Math.min(ds0, ds1); } if (ds > min) { min = ds; mob = f_; if (ds0 > ds1) { mp = [pnt[3], pnt[4], pnt[5]]; } else { mp = [pnt[0], pnt[1], pnt[2]]; } }
        }
        ...
        const lrc = lr(mob.color, lc); mg[i][j] = {"r": Math.round(lrc.r), "g": Math.round(lrc.g), "b": Math.round(lrc.b)};
    }
}
